...
const Achievement = () => {
  ...
  return (
    <div id="achievement" className="px-4 sm:px-8 md:px-30 py-8">
      <div className="bg-white/60 rounded-3xl p-8 border border-black/10 shadow-lg flex-1">
        ...
        <div>
          {memoizedAchievements.map((item: AchievementType, index: number) => (
            <div
            ...
            >
              <div className="flex items-center w-full lg:w-auto mb-4 lg:mb-0">
                ...
                <div className="flex-1 flex flex-col justify-center relative z-10">
                  <h4 className="text-lg lg:text-xl font-semibold text-blue-600 m-0 mb-1">
                    {item.title}
                  </h4>
                  <p className="text-sm lg:text-base text-gray-500 m-0 leading-snug">
                    {item.description}
                  </p>
                </div>
              </div>
              <div className="relative z-10 flex-shrink-0 text-sm lg:text-base text-gray-500 font-medium min-w-[100px] text-left lg:text-right">
                {item.date}
              </div>
            </div>
          ))}
